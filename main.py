...
def get_total_participants():
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM users WHERE telegram = 1 OR instagram = 1 OR (telegram = 1 AND instagram = 1)"
    )
...
async def check_subs(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await callback.message.answer(
        f'Подписка в телеграме: {tg_status}\nПроверку подписки в инстаграме проведем перед розыгрышем, пока напиши свой никнейм в инстаграме или нажми кнопку "Пропустить":',
        reply_markup=skip_sub_check_kb
    )
    await callback.answer()
...
async def list_command(message: types.Message):
    ...
    await message.answer(
        f'Если вы провели розыгрыш и мне нужно отправить сообщению победителю, используйте команду /send_msg'
    )
...
async def results_lottery(message: types.Message, bot: Bot, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            'Не удалось отправить сообщение победителю.\nВозможно, он заблокировал бот.\n\nВыберите нового победителя и пришлите ответным сообщением его id:'
        )


async def main():
    logging.info('Start http server http://0.0.0.0:5000')
    server = HTTPServer(("0.0.0.0", 5000), WebRequestHandler)
